...
//11.26
    if (fabs(last_key_frame_pose(0,3) - current_frame_.pose(0,3)) + 
        ...
        fabs(last_key_frame_pose(2,3) - current_frame_.pose(2,3)) <key_frame_distance_){
            // std::cout<<current_frame_.pose<<std::endl;
            num1=last_key_frame_pose(0,3)*current_frame_.pose(0,3)+last_key_frame_pose(1,3)*current_frame_.pose(1,3)+last_key_frame_pose(2,3)*current_frame_.pose(2,3);
           // num1=last_key_frame_pose(:,3)*current_frame_.pose(:,3);
           // denom1=last_key_frame_pose(0,3).norm*current_frame_.pose(:,3).norm;
            norm1=sqrt(pow(last_key_frame_pose(0,3),2)+pow(last_key_frame_pose(1,3),2)+pow(last_key_frame_pose(2,3),2));
            norm2=sqrt(pow(current_frame_.pose(0,3),2)+pow(current_frame_.pose(1,3),2)+pow(current_frame_.pose(2,3),2));
            cossim=0.5+0.5*(num1/(norm1*norm2));
            if  (cossim>=0.5) 
            UpdateWithNewFrame(current_frame_);
            last_key_frame_pose = current_frame_.pose;
           // std::cout<<current_frame_.pose<<std::endl;
    ...
    //Difference cosineSimilarity12.26
    // if (fabs(last_key_frame_pose(0,3) - current_frame_.pose(0,3)) + 
    //     fabs(last_key_frame_pose(1,3) - current_frame_.pose(1,3)) +
    //     fabs(last_key_frame_pose(2,3) - current_frame_.pose(2,3)) == key_frame_1 ){
    //         num1=last_key_frame_pose(0,3)*current_frame_.pose(0,3)+last_key_frame_pose(1,3)*current_frame_.pose(1,3)+last_key_frame_pose(2,3)*current_frame_.pose(2,3);
    //        // num1=last_key_frame_pose(:,3)*current_frame_.pose(:,3);
    //        // denom1=last_key_frame_pose(0,3).norm*current_frame_.pose(:,3).norm;
    //         norm1=sqrt(pow(last_key_frame_pose(0,3),2)+pow(last_key_frame_pose(1,3),2)+pow(last_key_frame_pose(2,3),2));
    //         norm2=sqrt(pow(current_frame_.pose(0,3),2)+pow(current_frame_.pose(1,3),2)+pow(current_frame_.pose(2,3),2));
    //         cossim1=0.5+0.5*(num1/(norm1*norm2));
    // }
    //     if (fabs(last_key_frame_pose(0,3) - current_frame_.pose(0,3)) + 
    //     fabs(last_key_frame_pose(1,3) - current_frame_.pose(1,3)) +
    //     fabs(last_key_frame_pose(2,3) - current_frame_.pose(2,3)) == key_frame_2 ){
    //         num2=last_key_frame_pose(0,3)*current_frame_.pose(0,3)+last_key_frame_pose(1,3)*current_frame_.pose(1,3)+last_key_frame_pose(2,3)*current_frame_.pose(2,3);
    //         norm3=sqrt(pow(last_key_frame_pose(0,3),2)+pow(last_key_frame_pose(1,3),2)+pow(last_key_frame_pose(2,3),2));
    //         norm4=sqrt(pow(current_frame_.pose(0,3),2)+pow(current_frame_.pose(1,3),2)+pow(current_frame_.pose(2,3),2));
    //         //denom2=last_key_frame_pose(:,3).norm*current_frame_.pose(:,3).norm;
    //        cossim2=0.5+0.5*(num2/(norm3*norm4));
    // }
    //   if((cossim1>cossim2)&& fabs(last_key_frame_pose(0,3) - current_frame_.pose(0,3)) + 
    //     fabs(last_key_frame_pose(1,3) - current_frame_.pose(1,3)) +
    //     fabs(last_key_frame_pose(2,3) - current_frame_.pose(2,3)) ==  key_frame_1 )
    //         UpdateWithNewFrame(current_frame_);
    //         last_key_frame_pose = current_frame_.pose;
